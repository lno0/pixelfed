...
While ```git``` is included with most Linux distributions you will have to install both ```docker``` and ```nginx``` on your own. 
...
You will also need the packages ```php8.1-cli``` and ```php-fpm```, install these from the package manager from your distro. 
...
And finally cache your configuration (you need to do this every time you change your .env file):
```bash
...
fastcgi_pass unix:PATH_TO_PHPFPM; # make sure this is correct
...
We will be using Certbot for this. This is really simple, use the following command:
